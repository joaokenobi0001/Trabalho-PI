...
import { validarCRP, validarDivulgacaoEtica } from '../utils/validacoesCFP';
...
const FormCadastroPsicologo = () => {
  const navigate = useNavigate();
  const { adicionarPsicologo } = usePsicologos();
  ...
  const validateForm = () => {
    ...
    else if (!validarCRP(formData.crp)) newErrors.crp = 'CRP inválido (formato NN/XXXXX)';
    ...
    if (formData.modalidade === 'presencial' && (!formData.endereco || !formData.cidade)) {
      newErrors.endereco = 'Para atendimento presencial, informe endereço e cidade';
    }
    
    // Validação ética da divulgação
    const especializacoesArray = formData.especializacoes.split(',').map(s => s.trim());
    const divulgacaoEtica = validarDivulgacaoEtica(formData.descricao, especializacoesArray, formData.abordagens);
    if (!divulgacaoEtica.valido) {
      newErrors.etica = divulgacaoEtica.motivo;
    }
  ...
  const handleSubmit = (e) => {
    ...
    adicionarPsicologo(novoPsicologo);
    setSuccessMsg('Cadastro realizado com sucesso! Aguardando validação por nossos professores de psicologia. Você será notificado por e-mail em até 5 dias úteis.');
    setTimeout(() => {
      navigate('/');
    }, 3000);
  };
  ...
  return (
    <div className={styles.formContainer}>
      ...
      <p className={styles.formDisclaimer}>
        Todos os cadastros são revisados manualmente por professores de psicologia e psicólogos atuantes para garantir aderência ética e legal.
      </p>
      ...
      <form onSubmit={handleSubmit} className={styles.cadastroForm}>
        ...
        <div className={styles.formGroup}>
          ...
          <input type="text" name="formacao" value={formData.formacao} onChange={handleChange} placeholder="Ex: Mestre em Psicologia Clínica pela USP" />
          {errors.formacao && <span className={styles.error}>{errors.formacao}</span>}
        </div>

        <div className={styles.formGroup}>
          <label>Especializações (separadas por vírgula)</label>
          <input type="text" name="especializacoes" value={formData.especializacoes} onChange={handleChange} placeholder="Ansiedade, Depressão, TCC" />
        </div>
        ...
        {formData.modalidade === 'online' && (
          <div className={styles.formRow}>
            <div className={styles.formGroup}>
              ...
              <input type="text" name="cidade" value={formData.cidade} onChange={handleChange} placeholder="Sua cidade base" />
            </div>
            <div className={styles.formGroup}>
              <label>Estado</label>
              <input type="text" name="estado" value={formData.estado} onChange={handleChange} placeholder="UF" maxLength="2" />
            </div>
          </div>
        )}
        ...
        <div className={styles.formGroup}>
          <label>Descrição profissional (é permitida divulgação ética, sem promessas de cura) *</label>
          <textarea name="descricao" rows="4" value={formData.descricao} onChange={handleChange} placeholder="Fale sobre sua atuação, público-alvo e diferenciais..."></textarea>
        </div>

        {errors.etica && <div className={styles.errorBox}>{errors.etica}</div>}

        <div className={styles.formGroup}>
          <div className={styles.termos}>
            <input type="checkbox" required id="termos" />
            <label htmlFor="termos">Declaro que as informações fornecidas são verdadeiras, estou de acordo com o Código de Ética do CFP e autorizo a plataforma a divulgar meus dados conforme a LGPD.</label>
          </div>
        </div>

        <button type="submit" className={styles.btnSubmit}>Cadastrar e Aguardar Validação</button>
      </form>
    </div>
  );
};

export default FormCadastroPsicologo;